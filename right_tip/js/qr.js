function addLR(imgUrlR = [], qrUrlR = []) {
  ...
  function adRight(param, hover) {
    ...
    for (var i = 0; i < param.length; i++) {
      a_html += '<a ' + 'id="' +
        ...
        '<div style="display:none;position: absolute;bottom: 0;left: -136px"><img style="width: 136px;" src="' +
        hover[i] +
        '"></div>' +
        '<img' +
        ' src="' +
    ...
    html = '<div id="adRight" style="position:absolute;width:82px;height:' +
      82 * param.length +
      'px;z-index:10001">' +
      '<span id="closeBtnR" style="line-height: 16px;z-index:30;margin: 5px;position:absolute;top:-30px;right:0;cursor: pointer;color: #aaa" href="javascript:void(0);"><i style="margin-right: 4px;font-size:12px;vertical-align:bottom;line-height: 8px;text-align:center;display: inline-block;width: 12px;height: 12px;font-style: normal;border: 1px solid #aaa;-webkit-border-radius: 50%;-moz-border-radius: 50%;border-radius: 50%;">x</i>关闭</span>' +
      a_html +
      '</div>';
    dom.insertHtml(html);
  ...
  myEvent.add(window, 'load', adRight(imgUrlR, qrUrlR));//添加在右侧
}
